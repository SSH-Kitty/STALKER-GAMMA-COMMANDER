...
# The current Qt and vendored CLI payload raise the produced image's actual
# glibc floor to 2.34; see the README requirements section.
#
...
#       'commander_gui' next to wherever the user launched us cannot
...
install -Dm644 "$ICON_SRC" "$APPDIR/usr/share/icons/hicolor/256x256/apps/$APP_ID.png"
ln -sf "$APP_ID.png" "$APPDIR/.DirIcon"
install -Dm644 "$APPDIR/$APP_ID.desktop" "$APPDIR/usr/share/applications/$APP_ID.desktop"

# ------------------------------------------------------------------ package
VERSION="$(sed -n 's/^__version__ = "\(.*\)"/\1/p' "$PROJECT_DIR/commander_gui/__init__.py")"
